...
class_descriptions = [
    "TPrompt action with time sensitivity.",
    "Indicates that the user interface does not employ any dark patterns. In ethical design, this label suggests transparency and fairness.",
    "Describes tactics that imply limited availabilityof a product to encourage quicker decision-making.",
    ...
    "Highlights the use of social influence or statistics to persuade users by showing that others have taken a specific action",
    "Refers to deliberate challenges in the user journey, making it difficult for users to complete desired tasks.",
...
def scrape_and_follow_links(url, depth=1):
    ...
    if response.status_code == 200:
     ...
     tag=['p','li','div','h1','h2','h3','h4','h5','h6']
     m=[]
     c=0
     for i in tag:
                ...
                if predicted_label != 'Not Dark Pattern':
                    ...
                    for i in range(len(class_labels)):
                        if class_labels[i]== predicted_label:
                            ...
                            break
                    for i in m:
                        if h1_tag.text.strip() == i[5]:
                            flag=1
                            break
                    l.append("Flaged field" )
                    l.append(h1_tag.text.strip())
                    l.append(h1_tag.get('class'))
                    
                    if(l[7]==None or flag==1):
                        continue
                    else:
                       m.append(l)
